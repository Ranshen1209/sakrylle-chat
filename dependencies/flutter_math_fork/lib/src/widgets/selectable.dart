...
class SelectableMath extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final fontFamilyFallback = <String>[
      if (effectiveTextStyle.fontFamily?.isNotEmpty == true)
        effectiveTextStyle.fontFamily!,
      ...?effectiveTextStyle.fontFamilyFallback,
    ];
...
class InternalSelectableMathState extends State<InternalSelectableMath>
  ...
  void bringIntoView(TextPosition position) {
    ...
    final endpoint = getLocalEndpointForPosition(position.offset);
    rootRenderBox.showOnScreen(
      rect: Rect.fromLTWH(endpoint.dx, endpoint.dy - preferredLineHeight, 1,
          preferredLineHeight),
    );
  }
