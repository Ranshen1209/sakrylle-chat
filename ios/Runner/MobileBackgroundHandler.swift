...
final class MobileBackgroundHandler: NSObject, CLLocationManagerDelegate {
  ...
  func receive(_ url: URL) -> Bool {
    guard url.scheme == "sakrylle-chat", url.host == "conversation" else { return false }
  ...
  private func updateActivity(force: Bool) async {
    ...
    if #available(iOS 16.1, *) {
      var existing = activity as? Activity<SakrylleGenerationActivityAttributes>
      ...
      if !didCleanActivities || !enabled("liveActivitiesEnabled") || !inBackground {
        for other in Activity<SakrylleGenerationActivityAttributes>.activities where other.id != existing?.id {
      ...
      if let existing, isRunning(existing) {
        ...
        do {
          let attributes = SakrylleGenerationActivityAttributes(groupId: UUID().uuidString)
          let next: Activity<SakrylleGenerationActivityAttributes>
          if #available(iOS 16.2, *) {
          ...
          activityObserver = Task { @MainActor [weak self] in
            for await state in next.activityStateUpdates {
              ...
              if state == .dismissed || state == .ended {
                if (self.activity as? Activity<SakrylleGenerationActivityAttributes>)?.id == next.id {
  ...
  @available(iOS 16.1, *)
  private func isRunning(_ activity: Activity<SakrylleGenerationActivityAttributes>) -> Bool {
    if #available(iOS 16.2, *) {
  ...
  @available(iOS 16.1, *)
  private func contentState(task: BackgroundGenerationTask, finished: Bool) -> SakrylleGenerationActivityAttributes.ContentState {
  ...
  @available(iOS 16.1, *)
  private func end(_ target: Activity<SakrylleGenerationActivityAttributes>,
                   state: SakrylleGenerationActivityAttributes.ContentState?, immediately: Bool) async {
  ...
  private func returnStatus(_ result: @escaping FlutterResult) {
    ...
    if Self.liveActivitiesSupported, #available(iOS 16.1, *) {
      ...
      if let activity = activity as? Activity<SakrylleGenerationActivityAttributes> {
