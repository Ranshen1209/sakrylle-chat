...
class IncomingShareHandlerTests: XCTestCase {
  ...
  func testOtherDeepLinksDoNotTriggerShareImport() {
    ...
    let handler = IosIncomingShareHandler()
    handler.register(messenger: messenger)
    for url in ["sakrylle-chat://oauth-return", "sakrylle-chat://conversation/chat", "https://share", "sakrylle-chat://share/file"] {
...
class RunnerTests: XCTestCase {
  ...
  func testActivityConversationLinkIsBufferedForColdStartAndConsumedOnce() async {
    let handler = MobileBackgroundHandler()
    XCTAssertFalse(handler.receive(URL(string: "sakrylle-chat://unrelated/chat")!))
    XCTAssertTrue(handler.receive(URL(string: "sakrylle-chat://conversation/chat-123")!))
  ...
  func testLiveActivityCreationCompletionAndRetryAreSerialized() async throws {
    ...
    XCTAssertFalse(Activity<SakrylleGenerationActivityAttributes>.activities.contains { $0.activityState == .active })
    handler.prepareForTermination()
  }
  ...
  func testOrphanedLiveActivitiesAreEndedWhenRuntimeStartsDisabled() async throws {
    ...
    let state = SakrylleGenerationActivityAttributes.ContentState(
    ...
    let orphan = try Activity.request(attributes: SakrylleGenerationActivityAttributes(groupId: UUID().uuidString),
  ...
  func testSystemEndedActivityDoesNotReappearUntilANewRunStarts() async throws {
    ...
    let activity = try XCTUnwrap(Activity<SakrylleGenerationActivityAttributes>.activities.first {
