...
build_fakefsify() {
    ...
    # Meson makes fakefsify conditional on libarchive. Fail before generating
    # a build graph without that target, including on a clean CI runner.
    command -v pkg-config >/dev/null 2>&1 || log_error "pkg-config is required (brew install pkgconf)"
    local archive_prefix
    archive_prefix="$(brew --prefix libarchive)" || log_error "libarchive is required (brew install libarchive)"
    export PKG_CONFIG_PATH="$archive_prefix/lib/pkgconfig${PKG_CONFIG_PATH:+:$PKG_CONFIG_PATH}"
    pkg-config --exists libarchive || log_error "libarchive development files are unavailable"
    mkdir -p "$BUILD_DIR"
    cd "$ISH_DIR"
    ...
    if [ ! -f "$BUILD_DIR/build.ninja" ]; then
        ...
        # A prior failed configure may have cached libarchive as unavailable.
        env -u IPHONEOS_DEPLOYMENT_TARGET SDKROOT="$host_sdk" meson setup --reconfigure --clearcache "$BUILD_DIR"
    fi
