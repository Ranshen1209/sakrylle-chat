...
class McpProvider extends ChangeNotifier {
  ...
  Future<void> _load() async {
    ...
    final renamed = _servers
        .map(
          (server) =>
              server.id == 'kelivo_fetch' && server.name == '@kelivo/fetch'
              ? server.copyWith(name: '@sakrylle/fetch')
              : server,
        )
        .toList();
    if (renamed.asMap().entries.any(
      (entry) => entry.value != _servers[entry.key],
    )) {
      await _persistServers(renamed);
      _servers = renamed;
    }
