...
class SettingsProvider extends ChangeNotifier {
  ...
  static const String _migrationsVersionKey = 'sakrylle_migrations_version_v1';
  static const int _sakrylleApiBaseMigrationVersion = 1;
  ...
  Future<void> _load() async {
    ...
    var providerConfigsLoaded = true;
    ...
    // Legacy embedding normalization belongs to BusinessSettingsRouter. This
    // checkpoint is only for the fork's API base migration.
    if (providerConfigsLoaded &&
        (prefs.getInt(_migrationsVersionKey) ?? 0) <
      ...
      if (changed) {
        await prefs.setString(
          _providerConfigsKey,
          jsonEncode(
            _providerConfigs.map(
              (key, value) => MapEntry(key, _maskedProviderJson(value)),
            ),
          ),
        );
      }
    ...
    if (backgroundJson == null &&
        (prefs.containsKey('ios_background_generation_enabled_v1') ||
            prefs.containsKey('android_background_chat_mode_v1'))) {
      // Preserve explicit 1.1.x opt-ins once. New background capabilities stay
      // disabled; the removed BackgroundTasks switch remains in legacy data.
      final androidMode = prefs.getString('android_background_chat_mode_v1');
      _mobileBackground = MobileBackgroundSettings(
        iosEnabled:
            prefs.getBool('ios_background_generation_enabled_v1') == true,
        androidEnabled: androidMode == 'on' || androidMode == 'on_notify',
        notificationsEnabled:
            prefs.getBool('ios_background_notifications_enabled_v1') == true ||
            androidMode == 'on_notify',
        liveActivitiesEnabled:
            prefs.getBool('ios_live_activity_enabled_v1') == true,
      );
      await prefs.setString(
        _mobileBackgroundKey,
        jsonEncode(_mobileBackground.toJson()),
      );
    }
    if (backgroundJson != null) {
      try {
    ...
    if (_providerConfigs.isEmpty) {
      ...
      final seededConfigs = _providerConfigs.map(
        (key, config) => MapEntry(key, _maskedProviderJson(config)),
      );
  ...
  Future<void> setProviderConfig(String key, ProviderConfig config) async {
    ...
    // Persist secrets before publishing the configuration. A keychain failure
    // must remain visible to the caller and must not become a successful save.
    if (config.apiKey.isNotEmpty || (previous?.apiKey.isNotEmpty ?? false)) {
      await secure.setApiKey(key, config.apiKey);
    }
    if ((config.apiKeys?.isNotEmpty ?? false) ||
        (previous?.apiKeys?.isNotEmpty ?? false)) {
      await secure.setApiKeys(
        key,
        config.apiKeys?.map((e) => e.key).toList() ?? [],
      );
    }
    final next = {..._providerConfigs, key: config};
    await _preferences.setString(
      _providerConfigsKey,
      jsonEncode(
        next.map((key, value) => MapEntry(key, _maskedProviderJson(value))),
      ),
    );
  ...
  // Runtime configs contain resolved keys. Every settings write must mask all
  // providers, including those untouched by the current operation.
  Map<String, dynamic> _maskedProviderJson(ProviderConfig config) => config
      .copyWith(
        apiKey: config.apiKey.isEmpty ? '' : _secureKeyPlaceholder,
        apiKeys: config.apiKeys
            ?.map(
              (entry) => entry.copyWith(
                key: entry.key.isEmpty ? '' : _secureKeyPlaceholder,
              ),
            )
            .toList(),
      )
    ...
    try {
      ...
      for (final entry in _providerConfigs.entries) {
        ...
        if (config.apiKeys != null) {
          ...
          if (plainKeys.isNotEmpty) {
            final existing = await secure.getApiKeys(
              key,
              count: config.apiKeys!.length,
            );
            final values = [
              for (var i = 0; i < config.apiKeys!.length; i++)
                config.apiKeys![i].key == _secureKeyPlaceholder
                    ? existing[i]
                    : config.apiKeys![i].key,
            ];
            await secure.setApiKeys(key, values);
            needsPersist = true;
          }
        }
      }
      ...
      if (needsPersist) {
        ...
        final map = _providerConfigs.map(
          (k, v) => MapEntry(k, _maskedProviderJson(v)),
        );
